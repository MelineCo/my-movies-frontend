...
function Home() {
  ...
  const movies = moviesData.map((data, i) => {
    const posterPath = `https://image.tmdb.org/t/p/w220_and_h330_face${data.poster_path}`
    console.log(posterPath)
    const isLiked = likedMovies.some(movie => movie === data.title);
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview} poster={posterPath} voteAverage={data.vote_average} voteCount={data.vote_count}  />;
  });
